...
export default function Exp({match}) {
  ...
  return (
    ...
    <Form.Group>
      ...
      <Form.Control placeholder="username" name="username" onChange ={(e) => setUsername(e.target.value)} />
      <Form.Label style = {{float : "left"}}>Password</Form.Label>
      <Form.Control className="pass" type="password" placeholder="password" name="password" onChange ={(e) => setPassword(e.target.value)}/>
      <div style={{float : "left"}} onMouseLeave = {hidePass} onMouseEnter = {showPass}>show password</div>
      </Form.Group>
<br/>
  <Button style={{display : "inline"}} variant = "success" name="tutor"  type="submit" onClick = {handleS}>Login</Button>
</Form>
