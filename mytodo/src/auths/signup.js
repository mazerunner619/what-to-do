...
export default function Exp({ match }) {
  ...
  async function handleS(e) {
    ...
    if (info.username.length && info.password.length && info.cpassword.length) {
      if (info.password !== info.cpassword) alert("passwords do not match !");
      else {
        ...
        alert(data.message);
        if (data.status && data.outcome) hist.push("/");
      }
    } else {
  ...
  return (
    <div id="authPage">
      ...
      <Form>
        ...
        <Button variant="success" name="tutor" type="submit" onClick={handleS}>
          Signup
        </Button>
      </Form>
      <br />
